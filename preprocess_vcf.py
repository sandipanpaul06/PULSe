...
com1 = 'gunzip -c ./VCF_datasets/' + filenamegz + ' > ./VCF_datasets/' + filename
os.system(com1)


path_read = "./VCF_datasets/" + str(filename)
...
a = pd.DataFrame(a)
a.to_csv('./VCF_datasets/' + filename[:-4] + '.csv', index=False, header=False)

print('VCF to CSV: done')

com2 = 'rm ./VCF_datasets/' + filename
...
path_write = "./VCF_datasets/"+ filename[:-4] + "_HT" + '.csv'
...
com3 = 'rm ./VCF_datasets/' + filename[:-4] + '.csv'
os.system(com3)
...
subdiving files
'''
os.mkdir('./VCF_datasets/'+writeFolder)
...
for i in range(range_for):
	...
	if end <= A.shape[1]:
		fname = "./VCF_datasets/"+ writeFolder + '/'+ filename[:-4] + "_" + str(i+1) + ".csv"
